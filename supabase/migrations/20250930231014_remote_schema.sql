...
BEGIN
  RETURN QUERY
  SELECT 
    'https://jjqjvrojkapmtsmncaag.supabase.co/functions/v1/cron-webhook?task=full-pipeline&secret=[HIDDEN]'::text as current_webhook_url,
    CASE 
...
BEGIN
  ...
  webhook_url := 'https://jjqjvrojkapmtsmncaag.supabase.co/functions/v1/cron-webhook';
  cron_secret := 'postgres-cron-secret-2024'; -- This matches our Supabase secret
  ...
  BEGIN
    ...
    VALUES ('cron_trigger', 'success', 'HTTP webhook request sent to fetch-mail-threads', 
            jsonb_build_object(
              ...
              'webhook_url', 'https://jjqjvrojkapmtsmncaag.supabase.co/functions/v1/cron-webhook?task=fetch-mail-threads&secret=[HIDDEN]',
              'method', 'http_get'
            ), NOW());
...
BEGIN
  ...
  webhook_url := 'https://jjqjvrojkapmtsmncaag.supabase.co/functions/v1/cron-webhook';
  cron_secret := 'postgres-cron-secret-2024';
  ...
  BEGIN
    ...
    VALUES ('cron_full_pipeline', 'success', 'HTTP webhook request sent for full pipeline execution', 
            jsonb_build_object(
              'request_id', request_id,
              'webhook_url', 'https://jjqjvrojkapmtsmncaag.supabase.co/functions/v1/cron-webhook?task=full-pipeline&secret=[HIDDEN]',
              'method', 'http_get',
