...
INSERT INTO weekly_summaries (week_start_date, week_end_date, summary_content, top_discussions, total_posts, total_participants) VALUES
  (
    ...
    '# PostgreSQL Weekly Summary - Week of ' || (NOW() - INTERVAL '7 days')::date || '
...
- **Posts**: 3
- **Participants**: 3
- **Duration**: ' || (NOW() - INTERVAL '2 days')::date || ' - ' || (NOW() - INTERVAL '12 hours')::date || '
...
- **Posts**: 3
- **Participants**: 3
- **Duration**: ' || (NOW() - INTERVAL '4 days')::date || ' - ' || (NOW() - INTERVAL '2 days')::date || '
...
- **Posts**: 2
- **Participants**: 2
- **Duration**: ' || (NOW() - INTERVAL '3 days')::date || ' - ' || (NOW() - INTERVAL '2 days')::date || '
